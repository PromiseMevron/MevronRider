...
@HiltViewModel
class ReferalDetailViewModel @Inject constructor (private val repository: MevronRepo) : ViewModel() {

    fun getReferal(data: ReferalReport): LiveData<GenericStatus<GeneralResponse>> {

        val result = MutableLiveData<GenericStatus<GeneralResponse>>()

        CoroutineScope(Dispatchers.IO).launch {
            try{
                val response = repository.getReferralReport(data)
